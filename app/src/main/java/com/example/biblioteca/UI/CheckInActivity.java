...
public class CheckInActivity extends AppCompatActivity {
    ...
    private void checkOperationResult(){
        if(exception == false) {
            ...
            Toast.makeText(CheckInActivity.this,exceptionMessage,Toast.LENGTH_SHORT).show();
        }
    }
    private void postCheckIn2WS(String isbn, String userId) {
        new Thread() {
            public void run() {
                ...
                RequestsService.addCheckIn(isbn,userId, CheckInActivity.this);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        pdRing.setVisibility(ProgressBar.INVISIBLE);
