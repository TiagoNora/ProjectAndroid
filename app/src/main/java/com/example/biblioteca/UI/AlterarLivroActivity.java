...
public class AlterarLivroActivity extends AppCompatActivity {
    ...
    private void checkOperationResult(){
        if(exception == false) {
            ...
            Toast.makeText(AlterarLivroActivity.this,exceptionMessage,Toast.LENGTH_SHORT).show();
        }
    }
    private void putBook2WS(StockDTO stockDTO, String isbn) {
        new Thread() {
            public void run() {
                ...
                RequestsService.loadBook(isbn,AlterarLivroActivity.this);
                RequestsService.updateBook(stockDTO,isbn, AlterarLivroActivity.this);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        pdRing.setVisibility(ProgressBar.INVISIBLE);
